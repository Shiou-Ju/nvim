...
-- 終端全螢幕 tab ↔ 底部 split 雙向切換（Shiou-Ju/nvim#96）do
-- 一般視窗維持原生 Ctrl+w T；此函數專為終端工作流：狀態感知、單鍵往返
local function toggle_terminal_maximize()
  ...
  -- 不在終端視窗：找既有 terminal buffer 開全螢幕 tab，沒有就開新終端
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    if vim.bo[buf].buftype == 'terminal' then
      vim.cmd('tabnew')
      vim.cmd('buffer ' .. buf)
...
-- 僅綁 normal mode：terminal mode 的空白鍵須保留給 shell（同 <leader>tt 的取捨）
vim.keymap.set({'n'}, '<leader>tm', toggle_terminal_maximize, { desc = '終端全螢幕 tab ↔ 底部 split 切換', noremap = true })
-- 終端全螢幕切換 end
